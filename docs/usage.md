...
A profile's justifications reference the subscription catalog's columns. For example, `opencode-go/deepseek-v4-pro` appears as the balanced profile's `sdd-apply` primary because, per `subscriptions/opencode-go/2026-08.md`, it costs $0.435/1M input, has 1M context, "think modes" reasoning, and is described as the "workhorse of the stack". Cross-reference the two files when you want to understand *why* an assignment was made. See [File reference](./file-reference.md) for the exact columns of each.
